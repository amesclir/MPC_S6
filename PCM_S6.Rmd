...
Mark Pagel (1994) diseñó un modelo para comprobar la evolución dependiente de dos caracteres binarios. Lo llamamos a este modelo el model Pagel94.
...
Para ver estos modelos vamos a utilizar los datos publicados originalmente por Benun Sutton & Wilson (2019). En este estudio los autores proponen que el cuidado parterno en peces oseos podría estar correlacionado con un caracter comportamental llamado pareja de desove que implica monogamia.
...
Podemos apreciar como el ajuste del modelo dependiente es bastante mejor que el ajuste del modelo indepediente. Esto nos permite concluir que estos dos rasgos están evolutivamente correlacionados.

Veamos una representación del modelo.
...
Para ilustrar este modelo vamos a utilizar data sets de los lagartos del género Anolis (Maghler et al., 2010; Insgram et al., 2016). Concretamente del color de la papada de los lagartos.
...
Nuestra hipótesis es que podríamos estar ante un modelo con tasas ocultas.
...
##Now that we have our data for hind foot toe number, we can proceed to convert it to the binary condition "limbed" (for toes>0) and "limbless" (otherwise)
...
El mejor modelo es el más simple. Y el segundo mejor el limbless hidden.
...
Carguemos los datos, observemoslos y preparemoslos para los análisis. Fíjate que para usar corHMM hemos puesto los datos de polén en un formato concreto.
