...
Sgiguiendo este método ajustamos dos modelos diferente y los comparamos. El primero de ellos es el modelo independiente que básicamente es ajustar el modelo Mk de manera independiente a nuestros dos rasgos binarios. La probabilidad del modelo independiente es justo el producto de la probabilidad de cada modelo (o lo que es lo mismo la suma de los logaritmos de las probabilidades).
El segundo modelo que ajustamos es el modelo dependiente en el que el estado de caracter de un rasgos binario depende el otro y viceversa.
...
Podemos apreciar como el ajuste del modelo dependiente es bastante mejor que el ajuste del modelo indpediente. Esto nos permite concluir que estos dos rasgos están evolutivamente corrleacionados.
...
MODELOS CON MÚLTIPLES REGÍMENES

Esto lo podemos hacer al menos de dos manera diferentes.
En el primer método que vamos a estudiar propondremos diferentes regímenes para la evolución de nuestro caracter discreto y representaremos dichos regímenes en la filogenia (Revell et al., 2021). En realidad esto es muy parecido a lo que hicimos para comprobar diferenes regímenes del modelo OU en la clase anterior.

Para ilustrar este modelo vamos a utilizar data set de los lagartos del género Anolis (Maghler et al., 2010; Insgram et al., 2016). Concretametne del color de la papada de los lagartos.
...
Si teneos un caracter discreto binario con estados de caracter 1 y 0. Imagina que este rasgo no evoluciona de la misma manera en todos los clados del árbol filogenético. En cambio, hay clados donde evoluciona por ejemplo lentamente y otros en los que evoluciona mucho más rápido. Este modelo es capaz de molizar esas diferencias en la tasa evolutiva del rasgo a lo largo del árbol.
...
Pasemos ahora a ajutar nuestro modelo hidden-rates con datos reales. Usemos el set de datos del número de dedos en sqamatae.
Recordemos que el segundo mejor modelo apoyaba una tedencía direccional hacia la peridad de dedos.
...
Cómo modelo alternativo podemos considerar uno en el que sólo en el estado sin extremidades tenemos estados y tasas ocultas. Esto lo podemos cambiar con el argumetno ncat = c(1,2).
...
Luego ajustamos el modelo contrario donde hay estados y tasas ocultas solo en los organismos que si tienen extremidades.
```{r}
...
Representemos nuestros modelos.
```{r}
...
El mejor modelos es el más simple.


Vamos a ver otro ejemplo pero esta vez vamos a usar el la función coHMM del paquete con el mismo nombre (odel of Beaulieu et al., 2013, 2020).
Para ello usaremos lo datos de Williams et al., (2014) en la que los autores investigan la evolución del poen bicelular y tricelular. Usaremos un subconjunto de datos  para agelizar el proceso computacional.
...
1. El modelo de Pagel94 que hemos utilizado es en el que el caracter x depende de y viceversa. No obstante, también se puede ajustar este modelo de manera que x depende y y no al contrario, o bien que y depende de x y no al contrario. Ajusta esos modelos a nuestro caso de estudio del cuidado parental en peces óseos y compáralo con el modelo de Pagel original.
